#[cfg(feature = "logging")]
use crate::bs_debug;
// use rustls::internal::inappropriate_handshake_message;
use crate::conn::{CommonState};
use rustls::internal::check::inappropriate_handshake_message;
use rustls::internal::conn::{ConnectionRandoms};
use crate::conn::State;
use rustls::internal::hash_hs::HandshakeHashBuffer;
use rustls::internal::kx;
...
use rustls::internal::msgs::enums::{ECPointFormat, PSKKeyExchangeMode};
use rustls::internal::msgs::enums::{ExtensionType, HandshakeType};
use rustls::internal::msgs::handshake::{CertificateStatusRequest, ClientSessionTicket, SCTList};
use rustls::internal::msgs::handshake::{ClientExtension, HasServerExtensions};
use rustls::internal::msgs::handshake::{ClientHelloPayload, HandshakeMessagePayload, HandshakePayload};
use rustls::internal::msgs::handshake::{ConvertProtocolNameList, ProtocolNameList};
use rustls::internal::msgs::handshake::{ECPointFormatList, SupportedPointFormats};
use rustls::internal::msgs::handshake::{HelloRetryRequest, KeyShareEntry};
use rustls::internal::msgs::handshake::{Random, SessionID};
use crate::msgs::message::{Message, MessagePayload};
use rustls::internal::msgs::persist;
use crate::ticketer::TimeBase;
use rustls::{SupportedCipherSuite, ServerName};
...
fn emit_client_hello_for_retry(
    ...
    let (ticket, resume_version) = if let Some(resuming) = &resuming_session {
        match &resuming.value {
            persist::ClientSessionValue::Tls13(inner) => {
                ...
                // TODO unsupported atm. Fallthrough
            }
    ...
    let fill_in_binder =  if config.enable_tickets {
...
pub(super) fn process_alpn_protocol(
    ...
    debug!(
        ...
        common
            .alpn_protocol
            .as_ref()
            .map(|v| rustls::internal::BsDebug::new(v))
    );
    Ok(())
}
...
impl State<ClientConnectionData> for ExpectServerHello {
    fn handle(mut self: Box<Self>, cx: &mut ClientContext<'_>, m: Message) -> NextStateOrError {
        ...
        use rustls::ProtocolVersion::{DTLSv1_2};


        let server_version = if server_hello.legacy_version == DTLSv1_2 {
            server_hello
        ...
        let version = match server_version {
            ...
            DTLSv1_2 if self.config.supports_version(DTLSv1_2) => {
            ...
            _ => {
                ...
                let msg = match server_version {
                    DTLSv1_2  => "server's TLS version is disabled in client",
                    _ => "server does not support DTLS v1.2",
                };
        ...
        match suite {
            ...
            SupportedCipherSuite::DTls12(suite) => {
                ...
                dtls12::CompleteServerHelloHandling {
                    config: self.config,
                    resuming_session,
                ...
                .handle_server_hello(cx, suite, server_hello, false)
            }
        }
    }
}
